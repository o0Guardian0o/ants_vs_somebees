...
public class Bee extends Insect {
	...
	private boolean slowed; //inform if the bee is slowed (initialised at false)
	private int turn_slow; //keep the number of turns the bee still must be slowed
	private int counter; //
	private boolean stuned; //inform if the bee is stuned (initialised at false)
	...
	public void leavePlace() {
		...
		/*
		 * if (this.place.getAnt() != null) { if(this.place.getAnt() instanceof
		 * Containing) { Containing cont_ant = (Containing) this.place.getAnt();
		 * ((Ant) cont_ant).expUp(armor_init);
		 * cont_ant.getInsectIn().expUp(armor_init); } else {
	...
	/**
	 * The action of a bee which is slowed 
	 * @param colony 
	 */
